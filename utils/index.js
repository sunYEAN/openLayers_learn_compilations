...
// 百度坐标BG-09 转换为 Web墨卡托投影标准坐标3857
export const bdCovertTo3857 = (point) => {
    const gcj_02 = bd09togcj02(point[0], point[1]);
    // return gcj02towgs84(gcj_02[0], gcj_02[1]);
    return gcj_02;
}
...
export const excelToJson = {
    ...
    ['道路地理数据'] (sheet) {
        ...
        json = json.map(item => {
            const lonLat = item['评定单元地理数据'] || '';
            const points = lonLat.split(' ').map(pointStr => {
                return bdCovertTo3857([
                    Number(pointStr.split('|')[0]), // 这里原坐标是百度坐标系 “BD-09” ，需要转换为 3857 标准 web墨卡托投影
